...
<%
	...
	{	
		   ...
		   out.println("alert('Please Signup/ Login first.');");
	...
	{
		try{			
		Enumeration ar = request.getParameterNames();
				
					
		while(ar.hasMoreElements())
		{			
			String na =	ar.nextElement().toString();			
	...
	for(int i=0; i < cart.size(); i=i+2)
	{
		PreparedStatement stmt1 = con.prepareStatement("select * from Products where Short_Name = ?");
		String quantity = (String) cart.get(i);	
		String item = (String) cart.get(i+1);
		...
		if(stmt1 != null)
		{			
			PreparedStatement stmt2 = con.prepareStatement("insert into cart(name, username, quantity) values(?,?,?)");
			stmt2.setString(1,item);
			stmt2.setString(2, name);			
			stmt2.setString(3, quantity);
			
			//out.println(stmt2.toString());
			stmt2.executeUpdate();
...
%>
	...
	<!-- //navigation -->
	<!-- banner -->
	<div class="banner banner10" style="margin-bottom: 100px">
		<div class="container">			
		</div>
	</div>
	...
	<table id="cart" class="table table-hover table-condensed">
					...
					<tbody>
<%
	PreparedStatement stmt3 = con.prepareStatement("select * from cart, Products where Products.Short_Name = cart.name");
...
%>
						...
						<tr>
							<td data-th="Product">
								<div class="row">
									<div class="col-sm-2 hidden-xs"><img src="<%=rs.getString("Products.Image_Display") %>" alt="..." class="img-responsive"/></div>
									<div class="col-sm-10">
										<h4 class="nomargin"><%=rs.getString("Products.Short_Name") %></h4>										
									</div>
								</div>
							</td>
							<td data-th="Price"><%=rs.getString("Products.Discounted_Price") %></td>
							<td data-th="Quantity">
								<input type="number" class="form-control text-center" value="<%=rs.getString("cart.quantity") %>" readonly>
							</td>
							<%
								int q = Integer.parseInt(rs.getString("cart.quantity"));
							%>
							<td data-th="Subtotal" class="text-center"><%=q*rs.getInt("Products.Discounted_Price") %></td>
...
<script type="text/javascript">
	...
	alert("Bas kar bhai sach mein paise dene hain?");
		});
	});
</script>
							<td><input type="submit" value="Checkout" class="btn btn-success btn-block checkout1"></td>
						</tr>
					</tfoot>
				</table>
</div>
